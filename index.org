...
#+BEGIN_SRC R :results value :exports results :colnames yes :hline yes
  ...
  merge(t,sampledesign,by.x=c("village_id","stratum_number"),by.y=c("village_id","stratum"),all.x=T)->t
  t[is.na(multiplier),multiplier:=1]
...
#+BEGIN_SRC R :results value :exports results :colnames no
  ...
  t1[as.numeric(age)>=7,.(population=length(id)),keyby=.(village_name,sex)]->t1
  occupations[as.numeric(age)>=7,.(numberofworkers=length(id)),keyby=.(village_name,sex,occupation_category)]->t
  t[t1] ->t
