...
#+BEGIN_SRC R :results silent :exports none
  ...
  agrilabour[labour_days==999|is.na(labour_days)]->t


  unique(hhid[id %in% t$sno_id])

  library(ggplot2)
  subset(agrilabour,village_name=="जमालपुर शेखाँ")->t
  merge(agrilabour, by.x="wages_kind_commodity1_id",
  subset(agrilabour,!is.na(wages_kind_commodity1_quantity))->t
  subset(t,wages_total_earnings_cash!=999)->t
  subset(t,labour_days!=999)->t
  subset(t,!is.na(wages_total_earnings_cash))->t
  subset(t,!is.na(labour_days))->t
  subset(t,wage_category=="Daily labour")->t

  t$wage<-t$wages_total_earnings_cash/t$labour_days

  agrilabour[wages_total_earnings_cash



  ggplot(t,aes(x=sex,y=wage))+geom_boxplot()
  t[,.(count=length(labour_days)),wage_category]

  subset(t,wage_category=="Piece-rated workers")
