...
#+BEGIN_SRC R :results silent :exports none
  ...
  t[,.(sno_id,name,age,sex,labour_days,hours=work_hours/labour_days,cash=wages_total_earnings_cash),id]->t0
...
#+BEGIN_SRC R :results value :exports results :colnames yes :hlines yes
  library(spatstat)
  copy(casual_labour_final)->t
  t[,.(days=sum(standarddays)),keyby=.(village,household_number,sno_id,id,name,age,sex,multiplier)]->t
  t[,.(mediandays=round(weighted.median(days,multiplier))),.(village,sex)]->t
  dcast(t,village~sex)->t
  names(t)<-c("गाँव","महिलायें","पुरुष")
  t
