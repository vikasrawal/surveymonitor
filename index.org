...
#+BEGIN_SRC R :results silent :exports none
  ...
  member[,.(sno_id,id,person_number,age,sex,name)]->t
  ...
  t[,.(sno_id,name,age,sex,crop_id,crop_operation_code,crop_operation_code2,crop_operation_code3,crop_operation_code4,labour_days,hours=work_hours/labour_days,cash=wages_total_earnings_cash),id]->t0
...
#+BEGIN_SRC R :results silent :exports none
  ...
  data.table(dbReadTable(surveypg,"ruralsurvey_non_agricultural_manual_labouring_out"))->nonagri
  ...
  data.table(dbReadTable(surveypg,"ruralsurvey_income_from_salaries"))->salaries
  salaries[is.na(annual_earning)]->t
  merge(t,hhid,by.x="sno_id",by.y="id")->t
  #hhid[id %in% t$sno_id]
  member[,c(1,2,3,4,5)]->t1
  merge(t,t1,by.x="name_of_worker_id",by.y="id")->t
  t[,c(29,30,32,38,39,40,6,4,5,8,28,27)]->t
  write.table(t,file="~/temp/missingsalaries.csv",sep="|",row.names=FALSE)

  ## Rents, pensions

  data.table(dbReadTable(surveypg,"ruralsurvey_scholarships_pensions_rents_remittances"))->remittances

  remittances[is.na(annual_earnings)]->t
  hhid[id %in% t$sno_id]
  ...
  machinehire[is.na(gross_payment_received)]->t
  hhid[id %in% t$sno_id]
#+END_SRC
