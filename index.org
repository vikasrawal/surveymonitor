...
#+BEGIN_SRC R :results silent :exports none
  ...
  animalinventory[is.na(total_present_value)&animal_type=="Cow/bullock" & adult_or_calf=="Calf"&sex=="Male",total_present_value:=number*500]
  animalinventory[is.na(total_present_value)&animal_type=="Cow/bullock" & adult_or_calf=="Calf"&sex=="Female",total_present_value:=number*500]
  animalinventory[is.na(total_present_value)&animal_type=="Buffalo" & adult_or_calf=="Calf"&sex=="Male",total_present_value:=number*500]
  animalinventory[is.na(total_present_value)&animal_type=="Buffalo" & adult_or_calf=="Calf"&sex=="Female",total_present_value:=number*1000]
  animalinventory[is.na(total_present_value)&animal_type=="Cow/bullock" & adult_or_calf=="Adult"&sex=="Male",total_present_value:=number*1000]
  animalinventory[is.na(total_present_value)&animal_type=="Buffalo" & adult_or_calf=="Adult"&sex=="Male",total_present_value:=number*1000]
  animalinventory[!(animal_type=="Cow/bullock"|animal_type=="Buffalo"),sex:=NA]
  animalinventory[!(animal_type=="Cow/bullock"|animal_type=="Buffalo"),adult_or_calf:=NA]
  animalinventory[(animal_type=="Cow/bullock"|animal_type=="Buffalo")&is.na(sex),sex:="Female"]
  animalinventory[(animal_type=="Cow/bullock"|animal_type=="Buffalo")&is.na(adult_or_calf),adult_or_calf:="Adult"]
#+end_src

#+NAME: animal-data-cleaning-code
#+BEGIN_SRC R :results silent :exports none
  dbListTables(surveypg)
  data.table(dbReadTable(surveypg,"ruralsurvey_animal_resources_for_last_one_year_inventory"))->animalinventory
  data.table(dbReadTable(surveypg,"ruralsurvey_codeanimaltype"))->animaltype
  ...
  animalfeed0[price>maxprice,total_value:=quantity*maxprice][is.na(total_value),total_value:=quantity*medprice]
  animalfeed0[price>maxprice,price:=maxprice][is.na(total_value),price:=medprice]


  ##animalfeed0[feed_id==1&quantity_unit_id==10]->t
  ##ggplot(t,aes(x=feed_type,y=price))+geom_boxplot()+facet_wrap(~feed_type,scales="free_y")
  ...
  ,total_present_value:=number*10000
  animalinventory[is.na(total_present_value)&animal_type=="Cow/bullock" & adult_or_calf=="Adult"&sex=="Female"&age<7]

  animalinventory[is.na(total_present_value)]->t
  [animal_type_id>2]
  [,.(count=length(id)),.(animal_type,adult_or_calf,sex)]
  animalinventory[,.(price=median(total_present_value/number,na.rm=TRUE),count=length(id)),animal_type_id]

  ## [35] "ruralsurvey_animal_resources_sale_production"                   
  ## [36] "ruralsurvey_animal_resources_work_done_by_household_members"    
  ## [37] "ruralsurvey_animal_resources_milk"                              
  ## [38] "ruralsurvey_animal_resources_for_last_one_year_inventory"       
  ## [39] "ruralsurvey_animal_resources_feed"                              
  ## [42] "ruralsurvey_animal_resources_for_last_one_year_sale_disposal"   
  ## [43] "ruralsurvey_animal_resources_other_costs"                       
#+end_src
...
#+BEGIN_SRC R :results value :exports results :colnames yes :hlines yes
  copy(animalinventory)->t
  t[animal_type=="Cow/bullock"|animal_type=="Buffalo",animal_type:="Bovines"]
  t[animal_type=="Bovines"]->t
  t[,.(number=sum(number)),.(sno_id)]->t
  #t[,number:=1]
  ...
  t[,hh_animal_prop:=round(hh_animal/hhall,1)]
...
#+CAPTION: विभिन्न गाँवों में पशुपालन करने वाले परिवारों के पास औसत गाय/भैसों की संख्या, समुदाय के अनुसार
#+RESULTS: animal-inventory-table1-code
| गाँव            | दलित | मुस्लिम | पिछड़ी जातियाँ | अन्य |
|----------------+------+-------+--------------+-----|
| खंदरई           |  2.6 |     0 |          1.4 | 3.9 |
| चहड़ कलां        |  2.3 |     0 |          2.1 | 4.7 |
| भिरढाणा        |  3.4 |     3 |          2.6 | 3.4 |
| भिरढाणा ढाणियाँ |  3.4 |     0 |            5 | 4.8 |
| जमालपुर शेखाँ     |  2.5 |     0 |          2.5 | 5.8 |
| गिज्जी          |  2.3 |     0 |          3.1 | 4.1 |
| भादस           |  2.8 |   2.3 |          2.3 |   0 |
