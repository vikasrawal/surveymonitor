...
#+BEGIN_SRC R :results value :exports results :colnames yes :hline yes
  ...
  p[crop=="Paddy"|crop=="Wheat"|crop=="Cotton - Badi/Kapas"|crop=="Mustard"]->p
  p[main!=999&!is.na(main)&!is.na(area)&!is.na(unit_main)]->p
  ifelse(p$unit_main==10,p$main*100,p$main)->p$main_kg
  p[,.(Yield=main_kg/area),by=.(village_name,crop,household_number,qid)]->p
  p[Yield!=0]->p
  is_outlier <- function(x) {
      return(x < quantile(x, 0.25) - 1.5 * IQR(x) | x > quantile(x, 0.75) + 1.5 * IQR(x))
  }
  p[,outlier:=is_outlier(Yield),by=,.(village_name,crop)]->k
  k[,label:=ifelse(outlier,qid,"")]->k

  ggplot(k,aes(x=crop,y=Yield))->a3
  a3+geom_boxplot()+geom_text_repel(aes(label=label),hjust=-0.5)+scale_y_continuous(name="Kilograms per acre")
